...
client.on("message", (message) => {
    ...
    if (timestamps.has(message.author.id)) {
        ...
        if (now < expirationTime) {
            ...
            return message.reply(
                `Biraz Sakinleşmelisin ${timeLeft.toFixed(1)} Saniye Sonra \`${command.name}\` Komudunu Kullanabilirsin.`
            );
        }
    }
    ...
    try {
        ...
        console.error(error);
        message.reply("@Lasty#0001 Kodda Bir Hata Var Duzeltmen Gerekiyor.").catch(console.error);
    }
});
    ...
    if (talkedRecently.get(message.author.id)) {
        ...
        if (level.xp >= nextXP) {
           ...
           /* let embed = new Discord.MessageEmbed()
            ...
            try {
                client.channels.cache.get('910984836703998042').send(`<a:phantomgif:909553355586666517>**Tebrikler** ${message.author}! Seviye Atladınız Yeni Seviyeniz **${level.level}**<a:upup:910993920878592091>`)
            } catch (err) {
                message.channel.send(`<a:phantomgif:909553355586666517>Tebrikler, ${message.author}! Chatteki Etkileşimin Sayesin de **${level.level}**. Seviyeye Ulaştın <a:upup:910993920878592091>`)
            }
        };
...
var roleLogs = "910984836703998042"

client.on("guildMemberRoleAdd", async(member, role) => {
 let entry = await member.guild.fetchAuditLogs({type: 'GUILD_MEMBER_UPDATE'}).then(audit => audit.entries.first());
if (entry.executor.id != client.user.id) return;
client.channels.cache.get(roleLogs).send(`<a:phantomgif:909553355586666517>**Tebrikler** ${member}! Chatteki Etkileşimin Sayesin de Seviye Atladın ve \`${role.name}\` Rolüne Yükseldin. <a:upup:910993920878592091>`);
});
